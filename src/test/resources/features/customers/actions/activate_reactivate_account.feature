Feature: Activate and Reactivate Account Actions Functionality
  ...
  Scenario: 1. Verifying Activate account  DENIED Process action as Maker and Checker
    ...
    And I send a POST request to "ep_customer_action_reset_activate_maker" to activate the account using the user code "129676245406479" as a maker
    ...
    When I review a POST request to "ep_customer_action_reset_activate_checker" to activate the change request for the user code "129676245406479" with the status "DENIED" as the checker
    Then the response status code should be 200
  ...
  Scenario: 2. Verifying Activate account  AUTHORIZED Process action as Maker and Checker
    ...
    Given I have a valid access token for the user "bini2"
    And I send a POST request to "ep_user_passcode_login" with the passcode "Admin@7"
    When I review a POST request to "ep_customer_action_reset_activate_checker" to activate the change request for the user code "129676245406479" with the status "AUTHORIZED" as the checker
    Then the response status code should be 200
  ...
  Scenario: 3. Verifying Reactivate account  DENIED Process action as Maker and Checker
    Given I have a valid access token for the user "zolam"
    And I send a POST request to "ep_user_passcode_login" with the passcode "Abc@123"
    When I send a POST request to "ep_customer_action_reactivate_maker" to reactivate the account using the user code "129676245406479" as a maker
    Then the response status code should be 200
    ...
    Given I have a valid access token for the user "bini2"
    And I send a POST request to "ep_user_passcode_login" with the passcode "Admin@7"
    When I review a POST request to "ep_customer_action_reactivate_checker" to reactivate the change request for the user code "129676245406479" with the status "DENIED" as the checker
    Then the response status code should be 200
    And the response should contain a field named "message" with the value "OTP sending declined"
